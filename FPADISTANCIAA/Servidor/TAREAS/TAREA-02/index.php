...
if (count($almacenAgenda) != 0) {
    ...
    foreach ($almacenAgenda as $nombre => $telefono) {
        echo "<input type='text' value='$nombre'   size='18px' disabled class='visualizarAgenda'>";
        echo "<input type='text' value='$telefono' size='12px' disabled class='visualizarAgenda'><br>";
...
<fieldset>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get">
        ...
        <input type="text" id="campoTelefono" name="telefono"<br><br>
		<br>
...
if (count($almacenAgenda) > 0) {
    echo <<<TEXTO
        <fieldset><legend>Vaciar Agenda</legend>
			<a href="{$_SERVER['PHP_SELF']}?limpiar=1">
			<button class="botonVaciar" >Vaciar</button></a>
        </fieldset>
TEXTO;
}
?>
